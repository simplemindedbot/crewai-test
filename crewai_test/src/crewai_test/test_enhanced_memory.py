...
def test_enhanced_memory():
    ...
    memory_store = EnhancedMemoryStore(
        "test_enhanced_memory.json", "test_embeddings.index"
    )
    ...
    memory_store.store_fact(
        "researcher_agent",
        "Large Language Models showed 340% growth in enterprise adoption in 2024",
    )
    memory_store.store_fact(
        "validator_agent",
        "MIT Technology Review ranked as top-tier AI publication with 95% accuracy rating",
    )
    memory_store.store_fact(
        "coordinator_agent",
        "Research workflow completed with high confidence validation scores",
    )
    ...
    context = memory_store.get_relevant_context(
        "coordinator_agent", "machine learning validation", context_limit=3
    )
    ...
    insights = memory_store.get_cross_agent_insights(
        "validation sources", exclude_agent="validator_agent"
    )
    ...
    memory_store2 = EnhancedMemoryStore(
        "test_enhanced_memory.json", "test_embeddings.index"
    )
    ...
    success = (
        len(ai_results) >= 2
        and len(context) >= 1  # At least 1 context item is sufficient
        and embeddings_count >= 6
        and len(insights) >= 1
    )
